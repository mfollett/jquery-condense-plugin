...
*  moreEvent: name of the event that should be triggered when the moreText is
*             clicked. Default: expand.condensePlugin
*  lessEvent: name of the event that should be triggered when the lessText is
*             clicked. Default: condense.condensePlugin
*  eventProxy: a jQuery object to trigger the more and less events on, defaults
*              to the span.condensedParent generated by this plugin
*  debug: whether or not to print debugging messages, default: false
*  pickCondenser: a method that determines which condenser to use to condense
*                 the text, Default: method that always chooses default condenser
*                 To override this provide a method that takes 3 parameters:
*                   * an assoc array of available condensers (currently only
*                     available is 'default')
*                   * a jQuery wrapper object of the element currently being
*                     operated on
*                   * the options passed into this plugin
*                 The returned value should be a function that takes the jQuery
*                 wrapped object of the element and the options, this function
*                 should return a condensed clone of the element passed in.
*                 This method provides great power to change how this plugin
*                 works, new condensing strategies can be used without needing
*                 to replicate the overarching logic of the plugin that builds
*                 the DOM wrappers around the condensed/not-condensed text.
*                 Since this function simply returns another function any
*                 approach to condensing can be used.
*/
...
(function($) {
  ...
  $.fn.condense = function(options) {
    ...
    return this.each(function() {
      ...
      var clone = options.pickCondenser(condensationMethods, $this, options)($this,options);
  ...
  function guaranteeEventProxy(options, $def) {
      if(! options.eventProxy) {
        options.eventProxy = $def;
      }
  }
  ...
  var condensationMethods = {
    'default': function (elem, opts) {
    ...
    'simpleParagraphCondenser': function($elem, options) {
        var findFirstNonImageChild = function ($elem) {
            var selection;
            // we want to use contents() instead of first() or children()
            // because it won't ignore naked text nodes inside of the element
            $elem.contents().each(function(_i, child) {
                var $child = $(child);
                if(!selection && $.trim($child.text()) !== '') {
                    selection = $child;
                }
            });
            return selection;
        ...
        var $clone = $elem.clone();
        var $first = findFirstNonImageChild($clone);
        $($first.prevAll().add($first.nextAll())).each(function(_index, child) {
            var $child = $(child);
            if($.trim($(child).text()) !== '') {
                $child.remove();
            }
  ...
  $.fn.condense.defaults = {
    ...
    debug: true,
    pickCondenser: function(condensers) { return condensers['default']; }
  };
